...
namespace UnclutteringYourInbox.Models
{
    ...
    /// <summary>
    /// The one feature model (with noise).
    /// </summary>
    public class OneFeatureModel : ReplyToModelBase
    {
        #region Features
        /// <summary>
        ...
        public override void ConstructModel()
        {
            ...
            Range message = new Range(this.NumberOfMessages).Named("message");
            ...
            message.AddAttribute(new Sequential());

            // observed data
            this.FeatureValue = Variable.Array<double>(message).Named("featureValue").Attrib(new DoNotInfer());
            ...
            this.RepliedTo = Variable.Array<bool>(message).Named("repliedTo");

            // Loop over emails
            using (Variable.ForEach(message))
            {
                var score = (this.FeatureValue[message] * this.Weight).Named("score");

                var noisyScore = Variable.GaussianFromMeanAndVariance(score, this.NoiseVariance).Named("noisyScore");
                this.RepliedTo[message] = noisyScore > this.Threshold;
            }
